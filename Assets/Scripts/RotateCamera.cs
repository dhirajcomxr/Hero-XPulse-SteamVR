using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class RotateCamera : MonoBehaviour
{
    public Transform target; 
    public float rotationSpeed = 20; 

    private Touch touch0, touch1; 
    public float zoomSpeed = 2; 
    public float maxDistance = 10; 
    public float minDistance = 2; 

    public float maxXRotation = 30, minXRotation = 0;

    float touchpreserve, touchcur, zoom;

    [SerializeField] GameObject ObjectToRoate;
    private Touch touch;
    private Vector2 touchposition;
    private Quaternion rotationY;
    private float roationSpeed = 0.1f;

    [SerializeField]Camera main;
    //Vector2 first, second;
    public float prevPinchAmt = 0f;
    public float currentXRotation;

    //public float zoomSpeed = 0.5f;
    private void Start() {
        main = GetComponent<Camera>();
    }

    void Update() {
        if (Input.touchCount > 0) {
            touch = Input.GetTouch(0);
            if (touch.phase == TouchPhase.Moved) {
                rotationY = Quaternion.Euler(0f, -touch.deltaPosition.x * roationSpeed, 0f);
                ObjectToRoate.transform.rotation = rotationY * ObjectToRoate.transform.rotation;
            }
        }
        if (Input.touchCount == 2) {
            Touch touch1 = Input.GetTouch(0);
            Touch touch2 = Input.GetTouch(1);
            //Debug.Log("Drag");
            float currentDistance = Vector2.Distance(touch1.position, touch2.position);
            float deltaDistance = Vector2.Distance(touch1.position - touch1.deltaPosition, touch2.position - touch2.deltaPosition);
            float zoomAmount = (currentDistance - deltaDistance) * zoomSpeed * Time.deltaTime;
            Camera.main.fieldOfView = Mathf.Clamp(Camera.main.fieldOfView - zoomAmount, minDistance, maxDistance);

        }
    }
    public static float ClampAngle(float angle, float min, float max) {
        if (angle < -360F)
            angle += 360F;
        if (angle > 360F)
            angle -= 360F;
        return Mathf.Clamp(angle, min, max);
    }
}
